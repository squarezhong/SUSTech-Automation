...
相反，如果由控制器直接对线性时不变系统$G_0(s)$进行控制，则效果比较不错。因此，有一种方式是，在控制器后加上一个非线性部分$f^{-1}(u)$，也就是后面给先行部分函数的反函数。但是，当f较为复杂的时候，我们不一定能找到它的反函数。因此，有一种寻找其反函数的方法是，先对$f(u)$进行线性化，然后取线性化之后的函数的反函数，这样会容易很多。比如下图的控制系统，就是我们希望的系统
